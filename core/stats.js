...
(function (global) {
  ...
  // 夏普值／索提諾比率用的無風險利率：用台幣計價的年化無風險利率（約當臺銀
  // 1 年期定儲利率），除以一年的交易日數換算成「每日無風險利率」，才能跟
  // 「資金占用日報酬率」這個日尺度的序列相減。
  const RISK_FREE_RATE_ANNUAL_PCT = 1.7;
  ...
  function riskAdjustedReturns(cleanTrades) {
    ...
    // 年化無風險利率換算成每日尺度，才能跟「資金占用日報酬率」相減
    const dailyRiskFree = (RISK_FREE_RATE_ANNUAL_PCT / 100) / TRADING_DAYS_PER_YEAR;

    const meanRate = rates.reduce((s, r) => s + r, 0) / n;
    const excessMean = meanRate - dailyRiskFree;
    const variance = rates.reduce((s, r) => s + (r - meanRate) ** 2, 0) / (n - 1);
    const stdDev = variance > 0 ? Math.sqrt(variance) : 0.0;

    // 下檔偏差只計算低於「每日無風險利率」（最低可接受報酬率 MAR）的部分
    const downsideSqSum = rates.reduce((s, r) => s + Math.min(0.0, r - dailyRiskFree) ** 2, 0);
    const downsideDeviation = Math.sqrt(downsideSqSum / n);

    const sharpe = stdDev > 0 ? pyRound(excessMean / stdDev, 3) : null;
    const sortino = downsideDeviation > 0 ? pyRound(excessMean / downsideDeviation, 3) : null;
  ...
  // 根據各檔損益彙總有幾種股票，決定效益比要列前幾名：
  //   預設（不到 16 檔）：Top 3；超過 15 檔：Top 5；超過 25 檔：Top 10（上限）
  // 股票種類少的時候列太多名沒有意義，種類多的時候多列幾名比較看得出哪些突出。
  function decideEfficiencyTopN(stockCount) {
    if (stockCount > 25) return 10;
    if (stockCount > 15) return 5;
    return 3;
  }

  function findBestEfficiencyStocks(symbolRows, topN = null) {
    if (topN === null || topN === undefined) topN = decideEfficiencyTopN(symbolRows.length);
  ...
  global.Stats = {
    ...
    buildSummary, buildSymbolSummary, findBestEfficiencyStocks, decideEfficiencyTopN,
