...
// 給「用 AI 深入點評」按鈕用：暫存最近一次分析的 summary 跟大盤對應數據,
// 沒有大盤資料時是 null（連 LLM 路線也一起擋住）
let _lastSummaryForLlm = null;
let _lastCorrForLlm = null;

// LLM 點評的 API 金鑰存在 localStorage（只存在這台裝置的瀏覽器,不會上傳到
// 任何第三方;送 API 請求是瀏覽器直接連到 Anthropic,金鑰只出現在請求標頭）。
const LLM_API_KEY_STORAGE = "tradeAnalyzer.llmApiKey.v1";
function loadLlmApiKey() {
  try { return localStorage.getItem(LLM_API_KEY_STORAGE) || ""; } catch (e) { return ""; }
}
function saveLlmApiKey(key) {
  try {
    if (key) localStorage.setItem(LLM_API_KEY_STORAGE, key);
...
function buildAnalysisPayload() {
  ...
  // AI 點評需要大盤資料才有意義（點評會比較各項數據跟大盤的關係）。沒有大盤
  // 資料時不產生點評，前端那塊卡片整個不顯示。判斷依據：有大盤資料的進場筆數 > 0。
  const hasIndexData = (indexCorrelation["有大盤資料的進場筆數"] || 0) > 0;
  const commentary = hasIndexData
    ? Commentary.buildRuleBasedCommentary(summary, indexCorrelation) : null;
  // 暫存給「用 AI 深入點評」按鈕用（沒有大盤資料就清空，連 LLM 路線也一起擋住）
  _lastSummaryForLlm = hasIndexData ? summary : null;
  _lastCorrForLlm = hasIndexData ? indexCorrelation : null;
...
/* =========================================================
   AI 點評
   預設顯示規則引擎產生的點評（離線就能跑）；本機 localStorage 有設定 API 金鑰時，
   多一個「用 AI 深入點評」按鈕走 LLM 路線（瀏覽器直接 fetch Anthropic API）。
   ========================================================= */
...
function renderCommentary(commentary) {
  ...
  const tag = document.getElementById("commentaryModeTag");
  if (!commentary || !commentary.paragraphs || !commentary.paragraphs.length) {
    card.style.display = "none";
    return;
  }
  card.style.display = "block";

  body.innerHTML = commentary.paragraphs.map((p) => {
    if (p.startsWith("（以上") || p.startsWith("（以下")) {
      return `<p class="cm-disclaimer">${p}</p>`;
    }
    const m = p.match(/^【([^】]+)】([\s\S]*)$/);
    if (m) {
      return `<p><span class="cm-sub-label">【${m[1]}】</span>${m[2]}</p>`;
    }
    return `<p>${p}</p>`;
  }).join("");

  tag.textContent = commentary.mode === "llm" ? "由 AI 模型生成" : "由內建規則引擎產生（離線）";
}

// 依「本機有沒有設定金鑰」跟「這次有沒有產生點評（有大盤資料）」決定 AI 按鈕顯不顯示
function refreshLlmButton(hasCommentary) {
  const btn = document.getElementById("llmCommentaryBtn");
  if (!hasCommentary) { btn.style.display = "none"; return; }
  btn.style.display = loadLlmApiKey() ? "inline-block" : "none";
}

// 「用 AI 深入點評」：瀏覽器直接呼叫 Anthropic Messages API。成功就用 AI 的
// 點評取代規則引擎的，失敗就提示錯誤、保留原本的規則引擎點評不動。
async function runLlmCommentary() {
  const btn = document.getElementById("llmCommentaryBtn");
  const body = document.getElementById("commentaryBody");
  const tag = document.getElementById("commentaryModeTag");
  const prevHtml = body.innerHTML;
  const prevTag = tag.textContent;

  const apiKey = loadLlmApiKey();
  if (!apiKey) { alert("尚未設定 API 金鑰，無法使用 AI 深入點評。"); return; }
  if (!_lastSummaryForLlm) { alert("還沒有分析資料（或這次沒有大盤資料），請先匯入對帳單分析。"); return; }

  btn.disabled = true;
  body.innerHTML = `<p class="commentary-loading">AI 正在分析這批交易數據，請稍候…</p>`;
  tag.textContent = "";

  try {
    const prompt = Commentary.buildLlmPrompt(_lastSummaryForLlm, _lastCorrForLlm);
    const resp = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      headers: {
        ...
        // 允許瀏覽器直接呼叫（Anthropic 需要這個標頭才會回應跨來源的前端請求）
    ...
    if (!resp.ok) {
      ...
      throw new Error(`AI 服務回應錯誤（HTTP ${resp.status}）。${detail}`);
    }
    ...
    if (!text) throw new Error("AI 回應是空的，請稍後再試。");

    const paras = text.split(/\n\s*\n/).filter((s) => s.trim());
    renderCommentary({ mode: "llm", paragraphs: paras.length ? paras : [text] });
  } catch (err) {
    body.innerHTML = prevHtml;
    tag.textContent = `${prevTag}　·　AI 深入點評未成功：${err && err.message ? err.message : err}`;
  } finally {
    btn.disabled = false;
  }
}

// 「設定金鑰」：用 prompt 讓使用者貼上金鑰，存到 localStorage（只存在本機瀏覽器）
function setupLlmKey() {
  const key = window.prompt(
    "貼上你的 Anthropic API 金鑰以啟用 AI 深入點評。\n" +
    "金鑰只會存在這台裝置的瀏覽器（localStorage），送點評請求時瀏覽器直接連到 Anthropic，不會經過任何第三方伺服器。\n" +
    "（留空後按確定可清除已存的金鑰、停用此功能）"
  );
  if (key === null) return; // 取消
  saveLlmApiKey(key.trim());
  refreshLlmButton(!!(_lastSummaryForLlm));
  if (key.trim()) alert("金鑰已儲存，現在可以使用「用 AI 深入點評」了。");
  else alert("已清除金鑰，AI 深入點評功能已停用。");
}

document.getElementById("llmCommentaryBtn").addEventListener("click", runLlmCommentary);
document.getElementById("llmSettingsBtn").addEventListener("click", setupLlmKey);
...
function renderResult(data) {
  const {
    ...
    index_coverage, index_correlation, benchmark_curve, commentary,
  } = data;
  ...
  // AI 點評：有大盤資料時才會帶 commentary，沒有的話整塊卡片不顯示，
  // AI 按鈕也跟著不顯示（點評需要大盤數據才有比較基礎）
  renderCommentary(commentary);
  refreshLlmButton(!!(commentary && commentary.paragraphs && commentary.paragraphs.length));
  ...
  const capitalStats = [
    ...
    statCell("夏普值", summary["夏普值"] !== null ? fmt(summary["夏普值"], { decimals: 3 }) : "—", "neutral", "日報酬率，無風險利率 1.7%，未年化"),
  ...
  // 效益比 Top N（N 由 stats 依股票檔數決定：一般 3、超過 15 檔 5、超過 25 檔 10）
  const bestEfficiencyBody = document.getElementById("bestEfficiencyBody");
  const efficiencyRows = best_efficiency_stocks || [];
  const efficiencyHeader = document.getElementById("bestEfficiencyHeader");
  if (efficiencyHeader) {
    const topN = efficiencyRows.length || 3;
    efficiencyHeader.innerHTML =
      `效益比 Top ${topN}　<span style="font-weight:400; color:var(--text-dim);">用最少漲幅、最少持有天數賺最多的股票</span>`;
  }
  ...
  // 表格資料填好之後，設定浮動標題列（往下捲動時固定在頁首下方）。
  // 包在 try/catch 裡，萬一這段出錯也不會影響整個分析結果的顯示。
  try { setupStickyHeaders(); } catch (e) { console.error("浮動標題列設定失敗", e); }
}

/* =========================================================
   表格浮動標題列（純 JS 複本，不依賴 CSS sticky）

   做法：給每個表格複製一份標題列，掛在 <body> 底下、用 position:fixed 固定在
   頁首下方。捲動時用 getBoundingClientRect() 判斷原本的標題列有沒有捲出畫面，
   有就顯示複本、沒有就藏起來。

   為什麼這樣最可靠：
   - 複本掛在 body 底下，不在表格卡片裡，所以卡片的任何 overflow / clip 都
     影響不到它（CSS sticky 容易被祖先元素的這些屬性擋住而失效）。
   - getBoundingClientRect() 回傳的座標永遠相對視窗，不受裁切容器影響。
   - 捲動監聽用「捕獲階段」（addEventListener 第三個參數 true）：scroll 事件
     不會冒泡，但捕獲階段會從 window 往下傳到目標，所以不管是整個頁面在捲、
     還是某個內層容器在捲，這個監聽都抓得到，不會漏。
   ========================================================= */
function setupStickyHeaders() {
  ...
  tables.forEach((table) => {
    ...
    // 複本：每個表格只建立一次，重複分析時更新內容就好
    let clone = table.__stickyClone;
    if (!clone) {
      clone = document.createElement("table");
      clone.className = table.className + " sticky-thead-clone";
      clone.appendChild(thead.cloneNode(true));
      document.body.appendChild(clone);
      table.__stickyClone = clone;
    } else {
      clone.replaceChild(thead.cloneNode(true), clone.firstChild);
    }

    function sync() {
      // 每次捲動時重新量頁首高度：頁首高度可能因視窗寬度而換行改變，
      // 如果快取成固定值，頁首換行後複本會貼在錯誤位置。
      const headerH = headerEl ? headerEl.getBoundingClientRect().height : 61;

      const tableRect = table.getBoundingClientRect();
      const theadRect = thead.getBoundingClientRect();
      const theadH = theadRect.height;

      const shouldShow = tableRect.top < headerH && tableRect.bottom > headerH + theadH;

      if (shouldShow) {
        clone.style.top = headerH + "px";
        clone.style.left = tableRect.left + "px";
        clone.style.width = tableRect.width + "px";
        const srcCells = thead.rows[0] ? thead.rows[0].cells : [];
        const dstCells = clone.rows[0] ? clone.rows[0].cells : [];
        for (let i = 0; i < srcCells.length && i < dstCells.length; i++) {
          dstCells[i].style.width = srcCells[i].getBoundingClientRect().width + "px";
        }
    ...
    table.__stickySync = sync;
    sync();
  });
}

// 全域只綁一次捲動 / resize 監聽。scroll 用捕獲階段（第三個參數 true）。
function refreshAllStickyHeaders() {
  document.querySelectorAll(".table-card table").forEach((t) => {
    if (t.__stickySync) t.__stickySync();
  });
}
if (!window.__stickyHeadersBound) {
  window.addEventListener("scroll", refreshAllStickyHeaders, true);
  window.addEventListener("resize", () => {
    try { setupStickyHeaders(); } catch (e) { console.error(e); }
  });
  window.__stickyHeadersBound = true;
}
